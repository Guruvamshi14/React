...
export const CountriesList = ({countries, query, region}) => {
  ...
  return (
    <div className={`${styles.countriesList} ${ isDark ? stylesMain.dark : stylesMain.light } `}>
        {
            country.length === 0 ? (
               <ShimmerSimpleGallery card imageHeight={300} caption />
             ) : (
                country.filter(function(data){
                    return data.name.common.toLowerCase().includes(query.toLowerCase()) && data.region == region;
                })
                .map((data, index) => {
